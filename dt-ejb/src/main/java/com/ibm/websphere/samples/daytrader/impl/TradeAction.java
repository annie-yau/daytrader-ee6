...
package com.ibm.websphere.samples.daytrader.impl;
        ...
        if (TradeConfig.runTimeMode == TradeConfig.EJB3) {
            try {
                if (!(trade instanceof TradeSLSBLocal)) {
                    TradeSLSBLocal tradeSLSB = null;
                    InitialContext context = new InitialContext();
                    try {
                        tradeSLSB = (TradeSLSBLocal) context.lookup("java:comp/env/web/ejb/local/TradeSLSBBean");
                    } catch (Exception ex) {
                        Log.error("TradeAction:createTrade - Lookup of TradeSLSBLocal failed!!!");
                        ex.printStackTrace();
                        tradeSLSB = (TradeSLSBLocal) context.lookup("TradeSLSBBean");
                    }
            ...
            try {
                if (!(trade instanceof DirectSLSBLocal)) {
                    DirectSLSBLocal directSLSB = null;
                    InitialContext context = new InitialContext();
                    try {
                        directSLSB = (DirectSLSBLocal) context.lookup("java:comp/env/web/ejb/local/DirectSLSBBean");
                    } catch (Exception ex) {
                        Log.error("TradeAction:createTrade - Lookup of DirectSLSBLocal failed!!!");
                        ex.printStackTrace();
                        directSLSB = (DirectSLSBLocal) context.lookup("DirectSLSBBean");
                    }
